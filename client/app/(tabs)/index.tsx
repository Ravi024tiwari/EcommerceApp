...
export default function Home() {
  ...
  const router =useRouter()//here its use to navigate user 

  const [activeBannerIndex,setActiveBannerIndex] =useState(0)

  const [products,setProducts] =useState<Product[]>([]) //this product of this type of product its store into it
        ...
        showsVerticalScrollIndicator={false}>
           ...
           onScroll={(e)=>{
            const slide =Math.ceil(e.nativeEvent.contentOffset.
              ...
              if(slide !=activeBannerIndex){
                setActiveBannerIndex(slide) ;//here we update the current sliding banner index
              }
           ...
           >
              {BANNERS.map((banner,index)=>(
                <View key={index} className='relative w-full h-48
                     bg-gray-200 overflow-hidden' style={{width:width-32}}>
          ...
          <View className='flex-row justify-center mt-3 gap-2'>
            {BANNERS.map((_,index)=>(
              <View key={index} className={`h-2 rounded-full 
                ${index === activeBannerIndex ? 'w-6 bg-primary':
                   'w-2 bg-gray-300'}`}/>
            ))}
          </View>
          ...
          >
            {categories.map((cat:any)=>(
              <CategoryItems key={cat.id} item={cat} isSelected={false} onPress={()=>router.push({pathname:"/shop",
