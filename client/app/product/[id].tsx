...
export default function ProductDetails() {
  ...
  const [product, setProduct] = useState<Product | null>(null); //here we set that Product details type
  ...
  const isLiked = isInWishlist(product._id); //add this into the wishlist
  ...
  const handleAddToCart =()=>{
    ...
    addToCart(product,selectedSize || "");//here we add that item into the cart
  }
  return (
    <View>
      <ScrollView contentContainerStyle={{ paddingBottom: 100 }}>
        ...
        <View className="relative h-[450px] bg-gray-100 mb-6">
          ...
          <View className="absolute top-12 left-4 right-4 flex-row justify-between items-center z-10">
            ...
            {/**Heart function */}
            {/**Its same work as redux tool kit */}
            <TouchableOpacity
        ...
        <View className="p-5">
            ...
            {product.sizes && product.sizes.length >0 && (
                ...
                <View className="flex-row gap-3 mb-6 flex-wrap">
                    {product.sizes.map((size)=>(
                        <TouchableOpacity key={size} onPress={()=>setSelectedSize(size)} className={`w-12 h-12 rounded-full
                            items-center justify-center border ${selectedSize==size ?'bg-primary border-primary' :
                             'bg-white border-gray-100'}`}>
                                <Text className={`text-sm font-medium ${selectedSize===size ?'text-white':
                                    'text-primary'}`}>{size}</Text>
                             </TouchableOpacity>
                    ))}
            ...
            <Text className="text-base font-bold text-primary mb-2">Descriptions</Text>
      ...
      {/**Footer Here we add the products into the Cart if its not already added into it*/}
      <View className="absolute bottom-0 left-0 flex-row right-0 p-4 bg-white border-t border-gray-100">
            <TouchableOpacity onPress={handleAddToCart} className="w-4/5 bg-primary py-4 rounded-full items-center shadow-lg flex-row justify-center">
                ...
                <Text className="text-white font-bold text-base ml-2">Add to Cart</Text>{/**Functionlity to AddToCart Button */}
            </TouchableOpacity>
